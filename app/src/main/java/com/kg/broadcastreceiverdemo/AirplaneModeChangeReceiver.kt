...
class AirplaneModeChangeReceiver : BroadcastReceiver() {
    ...
    override fun onReceive(context: Context?, intent: Intent?) {
        ...
        if (isAirplaneModeEnabled) {
            ...
            Toast.makeText(context, "Airplane Mode Enabled Kaustav", Toast.LENGTH_LONG).show()
        } else {
            // showing the toast message if airplane mode is disabled
            Toast.makeText(context, "Airplane Mode Disabled Kaustav", Toast.LENGTH_LONG).show()
        }
    }
}